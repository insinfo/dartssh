...
class HttpResponse {
  ...
  HttpResponse(this.status, {this.text, this.reason, this.headers, this.contentStream, this.contentLength});
}
...
abstract class HttpClient {
  ...
  Future<HttpResponse> request(String url, {String? method, String? data, Map<String, String>? headers});
}
...
class HttpClientImpl extends HttpClient {
  ...
  HttpClientImpl({this.clientFactory, StringCallback? debugPrint, StringFilter? userAgent})
      : super(debugPrint: debugPrint) {
    clientFactory ??= () => UserAgentBaseClient(userAgent == null ? null : userAgent('HttpClientImpl'), http.Client());
  }
  ...
  Future<HttpResponse> request(String url, {String? method, String? data, Map<String, String>? headers}) async {
    ...
    HttpResponse ret = HttpResponse(uriResponse.statusCode, text: uriResponse.body);
    if (debugPrint != null) {
...
class SSHTunneledBaseClient extends http.BaseClient {
  ...
  Future<http.StreamedResponse> send(http.BaseRequest request) async {
    ...
    return http.StreamedResponse(
      response.contentStream as Stream<List<int>>,
      response.status!,
...
Map<String, String> addBasicAuthenticationHeader(Map<String, String> headers, String username, String password) {
  headers['authorization'] = 'Basic ' + base64.encode(utf8.encode('$username:$password'));
  return headers;
}

Future<SocketInterface> connectUri(Uri? uri, SocketInterface socket, {SocketFilter? secureUpgrade}) async {
  /// We might be asking the remote to open an SSH tunnel to [uri].
  Completer<String> connectCompleter = Completer<String>();
  socket.connect(uri, () => connectCompleter.complete(null), (error) => connectCompleter.complete('$error'));
  String connectError = await connectCompleter.future;
  if (connectError != null) throw FormatException(connectError);

  if (secureUpgrade != null && uri!.hasScheme && (uri.scheme == 'https' || uri.scheme == 'wss')) {
...
Future<HttpResponse> httpRequest(Uri uri, String method, SocketInterface socket,
  ...
  socket.listen((Uint8List? m) {
    ...
    if (headerText == null) {
      buffer.add(m!);
      int headersEnd = searchUint8List(buffer.data, Uint8List.fromList('\r\n\r\n'.codeUnits));
      ...
      if (headersEnd != -1) {
        ...
        headers = Map<String?, String?>.fromIterable(lines.skip(1),
            key: (h) => h.substring(0, h.indexOf(': ')), value: (h) => h.substring(h.indexOf(': ') + 2).trim());
        ...
        if (contentLength == 0) {
          if (debugPrint != null) {
            debugPrint('SSHTunneledBaseClient.socket.listen: Content-Length: 0, remaining=${buffer.data.length}');
          }
          contentController.close();
    ...
    if (contentRead >= contentLength) {
      if (debugPrint != null) {
        debugPrint('SSHTunneledBaseClient.socket.listen: done $contentRead / $contentLength');
      }
      contentController.close();
  ...
  socket.send('${method} /${uri.path} HTTP/1.1\r\n' +
      requestHeaders.entries.map((header) => '${header.key}: ${header.value}').join('\r\n') +
      '\r\n\r\n');
