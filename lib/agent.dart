...
mixin SSHAgentForwarding on SSHTransport {
  ...
  static void dispatchAgentRead(Channel channel, Uint8List msg, ChannelInputCallback handleAgentPacket) {
    channel.buf.add(msg);
    while (channel.buf.data.length > 4) {
      ...
      handleAgentPacket(channel, SerializableInput(input.viewOffset(input.offset, input.offset + agentPacketLen)));
  ...
  void handleAgentPacket(Channel channel, SerializableInput agentPacketS) {
    ...
    switch (agentPacketId) {
      ...
      case AGENTC_SIGN_REQUEST.ID:
        handleAGENTC_SIGN_REQUEST(channel, AGENTC_SIGN_REQUEST()..deserialize(agentPacketS));
        break;
  ...
  void handleAGENTC_SIGN_REQUEST(Channel channel, AGENTC_SIGN_REQUEST msg) {
    ...
    Uint8List sig = identity!.signMessage(Key.id(keyType), msg.data, getSecureRandom());
    if (sig != null) {
...
class AGENT_IDENTITIES_ANSWER extends AgentMessage {
  ...
  @override
  int get serializedSize => keys.fold(serializedHeaderSize, (v, e) => v + 8 + e.key.length + e.value.length);
...
class AGENT_SIGN_RESPONSE extends AgentMessage {
  ...
  @override
  void deserialize(SerializableInput input) => sig = deserializeStringBytes(input);
