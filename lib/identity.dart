...
class Identity {
  ...
  Ed25519Signature signWithEd25519Key(Uint8List m) =>
      Ed25519Signature(tweetnacl.Signature(null, ed25519!.secretKey).sign(m)!.buffer.asUint8List(0, 64));

  ECDSAKey getECDSAPublicKey() =>
      ECDSAKey(Key.name(ecdsaKeyType), Key.ellipticCurveName(ecdsaKeyType), ecdsaPublic!.Q!.getEncoded(false));
  ...
  RSASignature signWithRSAKey(Uint8List m) {
    ...
    signer.init(true, PrivateKeyParameter<asymmetric.RSAPrivateKey>(rsaPrivate!));
...
class RSAKey with Serializable {
  ...
  @override
  int get serializedSize => serializedHeaderSize + formatId.length + mpIntLength(e!) + mpIntLength(n!);
...
class ECDSAKey with Serializable {
  ...
  @override
  int get serializedSize => serializedHeaderSize + formatId!.length + curveId!.length + q!.length;
...
class ECDSASignature with Serializable {
  ...
  @override
  int get serializedSize => serializedHeaderSize + formatId!.length + mpIntLength(r!) + mpIntLength(s!);
...
bool verifyEd25519Signature(Ed25519Key publicKey, Ed25519Signature signature, Uint8List message) =>
...
bool verifyECDSASignature(int keyType, ECDSAKey publicKey, ECDSASignature signature, Uint8List message) {
  ECDSASigner signer = ECDSASigner(Key.ellipticCurveHash(keyType));
  ECDomainParameters curve = Key.ellipticCurve(keyType)!;
  signer.init(false, PublicKeyParameter(ECPublicKey(curve.curve.decodePoint(publicKey.q!), curve)));
...
bool verifyRSASignature(RSAKey publicKey, RSASignature signature, Uint8List message) {
  RSASigner signer = RSASigner(SHA1Digest(), '06052b0e03021a');
  //isaque adicionei esta linha
  var rand = SecureRandom();
  signer.init(
      false,
      ParametersWithRandom(
          PublicKeyParameter<asymmetric.RSAPublicKey>(asymmetric.RSAPublicKey(publicKey.n!, publicKey.e!)), rand));
  return signer.verifySignature(message, asymmetric.RSASignature(signature.sig!));
}
