...
import 'package:dartssh/socket_html.dart' if (dart.library.io) 'package:dartssh/socket_io.dart';
import 'package:dartssh/ssh.dart';
import 'package:dartssh/transport.dart';
import 'package:dartssh/websocket_html.dart' if (dart.library.io) 'package:dartssh/websocket_io.dart';
...
class SSHClient extends SSHTransport with SSHAgentForwarding {
  ...
  SSHClient(
    ...
    if (socket == null) {
      ...
      socket = (hostport!.hasScheme && (hostport.scheme == 'ws' || hostport.scheme == 'wss'))
          ? WebSocketImpl()
          : SocketImpl();

      socket!.connect(hostport, onConnected, (error) => disconnect('connect error'));
    }
  }
  ...
  void handlePacket(Uint8List packet) {
    ...
    switch (packetId) {
      case MSG_KEXINIT.ID:
        state =
            state == SSHTransportState.FIRST_KEXINIT ? SSHTransportState.FIRST_KEXREPLY : SSHTransportState.KEXREPLY;
      ...
      case MSG_USERAUTH_FAILURE.ID:
        handleMSG_USERAUTH_FAILURE(MSG_USERAUTH_FAILURE()..deserialize(packetS!));
        break;
      ...
      case MSG_USERAUTH_INFO_REQUEST.ID:
        handleMSG_USERAUTH_INFO_REQUEST(MSG_USERAUTH_INFO_REQUEST()..deserialize(packetS!));
        break;
      ...
      case MSG_CHANNEL_OPEN.ID:
        handleMSG_CHANNEL_OPEN(MSG_CHANNEL_OPEN()..deserialize(packetS!), packetS);
        break;

      case MSG_CHANNEL_OPEN_CONFIRMATION.ID:
        handleMSG_CHANNEL_OPEN_CONFIRMATION(MSG_CHANNEL_OPEN_CONFIRMATION()..deserialize(packetS!));
        break;

      case MSG_CHANNEL_OPEN_FAILURE.ID:
        handleMSG_CHANNEL_OPEN_FAILURE(MSG_CHANNEL_OPEN_FAILURE()..deserialize(packetS!));
        break;

      case MSG_CHANNEL_WINDOW_ADJUST.ID:
        handleMSG_CHANNEL_WINDOW_ADJUST(MSG_CHANNEL_WINDOW_ADJUST()..deserialize(packetS!));
        break;
  ...
  void sendDiffileHellmanInit() {
    ...
    if (KEX.x25519DiffieHellman(kexMethod)) {
      ...
      writeClearOrEncrypted(MSG_KEX_DH_GEX_REQUEST(dh.gexMin, dh.gexMax, dh.gexPref));
  ...
  void handleMSG_KEXDH_REPLY(int packetId, Uint8List packet) {
    if (state != SSHTransportState.FIRST_KEXREPLY && state != SSHTransportState.KEXREPLY) {
    ...
    if (packetId == MSG_KEX_ECDH_REPLY.ID && KEX.x25519DiffieHellman(kexMethod)) {
      fingerprint = handleX25519MSG_KEX_ECDH_REPLY(MSG_KEX_ECDH_REPLY()..deserialize(packetS!)) ?? fingerprint;
    } else if (packetId == MSG_KEXDH_REPLY.ID && KEX.ellipticCurveDiffieHellman(kexMethod)) {
      fingerprint = handleEcDhMSG_KEX_ECDH_REPLY(MSG_KEX_ECDH_REPLY()..deserialize(packetS!)) ?? fingerprint;
    } else if (packetId == MSG_KEXDH_REPLY.ID && KEX.diffieHellmanGroupExchange(kexMethod)) {
      handleDhGroupMSG_KEX_DH_GEX_GROUP(MSG_KEX_DH_GEX_GROUP()..deserialize(packetS!));
      return;
    } else {
      fingerprint = handleDhMSG_KEXDH_REPLY(MSG_KEXDH_REPLY()..deserialize(packetS!)) ?? fingerprint;
    }
  ...
  void handleMSG_USERAUTH_SUCCESS() {
    ...
    sessionChannel = Channel(localId: nextChannelId, windowS: initialWindowSize);
    ...
    writeCipher(MSG_CHANNEL_OPEN('session', sessionChannel!.localId, initialWindowSize, maxPacketSize));
  ...
  void handleMSG_USERAUTH_INFO_REQUEST(MSG_USERAUTH_INFO_REQUEST msg) {
    if (tracePrint != null) {
      tracePrint!('$hostport: MSG_USERAUTH_INFO_REQUEST prompts=${msg.prompts.length}');
    }
  ...
  void handleMSG_CHANNEL_REQUEST(MSG_CHANNEL_REQUEST msg) {
    if (tracePrint != null) {
      tracePrint!('$hostport: MSG_CHANNEL_REQUEST ${msg.requestType} wantReply=${msg.wantReply}');
    }
  }
  ...
  void handleMSG_CHANNEL_OPEN(MSG_CHANNEL_OPEN msg, SerializableInput? packetS) {
    ...
    if (msg.channelType == 'auth-agent@openssh.com' && agentForwarding!) {
      ...
      writeCipher(MSG_CHANNEL_OPEN_CONFIRMATION(channel.remoteId, channel.localId, channel.windowS, maxPacketSize));
    } else if (msg.channelType == 'forwarded-tcpip') {
      MSG_CHANNEL_OPEN_TCPIP openTcpIp = MSG_CHANNEL_OPEN_TCPIP()..deserialize(packetS!);
      Forward? forward = forwardingRemote == null ? null : forwardingRemote![openTcpIp.dstPort];
      if (forward == null || remoteForward == null) {
        ...
        Channel channel = acceptChannel(msg);
        remoteForward!(channel, forward.targetHost, forward.targetPort, openTcpIp.srcHost, openTcpIp.srcPort);
        writeCipher(MSG_CHANNEL_OPEN_CONFIRMATION(channel.remoteId, channel.localId, channel.windowS, maxPacketSize));
      }
    } else {
  ...
  void handleSessionStarted() {
    if (agentForwarding!) {
      writeCipher(MSG_CHANNEL_REQUEST.exec(sessionChannel!.remoteId, 'auth-agent-req@openssh.com', '', true));
    }
    ...
    if (startShell!) {
      writeCipher(MSG_CHANNEL_REQUEST.ptyReq(sessionChannel!.remoteId, 'pty-req', Point(termWidth, termHeight),
          Point(termWidth * 8, termHeight * 12), termvar, '', true));

      writeCipher(MSG_CHANNEL_REQUEST.exec(sessionChannel!.remoteId, 'shell', '', true));

      if ((startupCommand ?? '').isNotEmpty) {
        sendToChannel(sessionChannel!, utf8.encode(startupCommand!) as Uint8List);
      }
    }
  }
  ...
  void sendPassword() {
    ...
    wrotePw = true;
    if (userauthFail != 0) {
      writeCipher(MSG_USERAUTH_REQUEST(login, 'ssh-connection', 'password', '', pw, Uint8List(0)));
    } else {
      List<Uint8List?> prompt = List<Uint8List?>.filled(passwordPrompts, Uint8List(0));
      prompt.last = pw;
  ...
  void sendAuthenticationRequest() {
    if (identity == null) {
      ...
      writeCipher(
          MSG_USERAUTH_REQUEST(login, 'ssh-connection', 'keyboard-interactive', '', Uint8List(0), Uint8List(0)));
      ...
      Uint8List challenge = deriveChallenge(sessionId!, login!, 'ssh-connection', 'publickey', 'ssh-ed25519', pubkey);
      Ed25519Signature sig = identity!.signWithEd25519Key(challenge);
      writeCipher(MSG_USERAUTH_REQUEST(login, 'ssh-connection', 'publickey', 'ssh-ed25519', pubkey, sig.toRaw()));
      ...
      Uint8List challenge = deriveChallenge(sessionId!, login!, 'ssh-connection', 'publickey', keyType, pubkey);
      ECDSASignature sig = identity!.signWithECDSAKey(challenge, getSecureRandom()!);
      writeCipher(MSG_USERAUTH_REQUEST(login, 'ssh-connection', 'publickey', keyType, pubkey, sig.toRaw()));
      ...
      Uint8List challenge = deriveChallenge(sessionId!, login!, 'ssh-connection', 'publickey', 'ssh-rsa', pubkey);
      RSASignature sig = identity!.signWithRSAKey(challenge);
      writeCipher(MSG_USERAUTH_REQUEST(login, 'ssh-connection', 'publickey', 'ssh-rsa', pubkey, sig.toRaw()));
    }
  }
  ...
  void sendChannelData(Uint8List b) {
    if (loginPrompts != 0) {
      ...
      bool cr = b.isNotEmpty && b.last == '\n'.codeUnits[0];
      pw = appendUint8List(pw ?? Uint8List(0), viewUint8List(b, 0, b.length - (cr ? 1 : 0)));
      if (cr) sendPassword();
    } else {
  ...
  void setTerminalWindowSize(int w, int h) {
    ...
    writeCipher(MSG_CHANNEL_REQUEST.ptyReq(sessionChannel!.remoteId, 'window-change', Point(termWidth, termHeight),
        Point(termWidth * 8, termHeight * 12), termvar, '', false));
  }

  void exec(String command, {bool wantReply = true}) {
    assert(socket != null && sessionChannel != null);
    if (socket == null || sessionChannel == null) return;
    writeCipher(MSG_CHANNEL_REQUEST.exec(sessionChannel!.remoteId, 'exec', command, wantReply));
  }
}
...
class SSHTunneledSocketImpl extends SocketInterface {
  ...
  SSHTunneledSocketImpl(Uri url, String? login, String? key, String? password,
    ...
    client = SSHClient(
        ...
        hostport: url,
        login: login,
        getPassword: password == null ? null : () => utf8.encode(password) as Uint8List,
  ...
  @override
  void connect(Uri? address, VoidCallback connectCallback, StringCallback errorHandler,
  ...
  void openTunnel([String sourceHost = '127.0.0.1', int sourcePort = 1234]) {
    ...
    channel = client!.openTcpChannel(sourceHost, sourcePort, tunnelToHost, tunnelToPort, (_, Uint8List? m) {
